...
const char* const PAYCHECKCASH_CONF_FILENAME = "paycheckcash.conf";
const char* const PAYCHECKCASH_PID_FILENAME = "paycheckcashd.pid";
...
public:
    ...
    static inline std::pair<bool, std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "paycheckcashd -foo=bar
...
#else
    const char* pszModule = "paycheckcash";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PaycheckCash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PaycheckCash
    // Mac: ~/Library/Application Support/PaycheckCash
    // Unix: ~/.paycheckcash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PaycheckCash";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PaycheckCash";
#else
    // Unix
    return pathRet / ".paycheckcash";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", PAYCHECKCASH_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", PAYCHECKCASH_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure PaycheckCash Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("PaycheckCash Core") == std::string::npos) {
