...
#if defined(NDEBUG)
#error "PaycheckCash cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "PaycheckCash Signed Message:\n";
...
static bool AcceptToMemoryPoolWorker(const CChainParams& chainparams, CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx, bool* pfMissingInputs, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced, bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting) {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s PCC additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    CAmount nSubsidy = CAmount(nHeight > FORK_BLOCK ? 100 : 50) * COIN;

    // Subsidy is cut in half every 
    nSubsidy >>= halvings;
    return nSubsidy;
}
...
{
    RenameThread("paycheckcash-scriptch");
