PaycheckCash Core version 0.16.0 is now available from:

  <https://paycheckcashcore.org/bin/paycheckcash-core-0.16.0/>
...
installer (on Windows) or just copy over `/Applications/PaycheckCash-Qt` (on Mac)
or `paycheckcashd`/`paycheckcash-qt` (on Linux).
...
PaycheckCash Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows Vista and later. Windows XP is not supported.

PaycheckCash Core should also work on most other Unix-like systems but is not
...
PaycheckCash Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
paycheckcash data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile PaycheckCash Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µPCC" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- paycheckcash-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- paycheckcash-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 18332.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest paycheckcash.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.paycheckcash.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for paycheckcash-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to paycheckcash-cli to allow RPC password to be read from standard input (jharvell)
...
- #12035 `eeb6d52` Change µPCC to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_paycheckcash_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` paycheckcash_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.paycheckcash doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for paycheckcash-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in paycheckcash-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop paycheckcashd (OmeGak)
...
- #10529 `331352f` Improve paycheckcashd systemd service file (Flowdalic)
