...
###   paycheckcashcore.org and paycheckcash.org and compares them.
...
WORKINGDIR="/tmp/paycheckcash_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://paycheckcashcore.org"
HOST2="https://paycheckcash.org"
BASEDIR="/bin/"
VERSIONPREFIX="paycheckcash-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'paycheckcash-',
...
if [ $? -ne 0 ]; then
   echo "paycheckcash.org failed to provide signature file, but paycheckcashcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "paycheckcash.org and paycheckcashcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the PaycheckCash Core binary release signing key installed?"
   fi
